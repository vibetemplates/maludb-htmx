<?php
/**
 * /v1/notes/{id}/reopen-issue  (requirements.md §4.5)
 *
 *   POST  Reopen a closed issue-type note (clears issue_closed_at).
 *         409 if the note is not an issue (type != 'issue') or is not currently closed.
 */
...
switch ($_SERVER['REQUEST_METHOD']) {
    ...
    case 'POST': {
        ...
        db_exec(
            "UPDATE maludb_memory SET issue_closed_at = NULL, updated_at = now() WHERE memory_id = ?",
            [$id]
        );

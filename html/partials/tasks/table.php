...
<!-- Results Summary -->
<div class="d-flex justify-content-between align-items-center mb-3" id="task-table-summary">
    <div>
    ...
    <div class="d-flex align-items-center gap-2">
        <label for="task-per-page-select" class="text-muted small mb-0">Items per page:</label>
        <select id="task-per-page-select"
...
<?php if (empty($tasks)): ?>
    <!-- Empty State -->
    <div class="text-center py-5" id="task-empty-state">
...
<?php else: ?>
        ...
        <table class="table table-hover align-middle" id="tasks-table">
                ...
                <tr>
                    ...
                    <th style="width: 50px;">
                        ...
                        <a href="#"
                           ...
                           hx-get="/partials/tasks/table.php?<?= $sortData['url'] ?>"
                           hx-target="#task-list-table"
                           hx-include="#task-search-input, #task-filters select, #task-filters input">
                    ...
                    <th style="width: 120px;">
                        <?php $sortData = getSortUrl('category', $sort, $queryString); ?>
                        <a href="#"
                           class="text-decoration-none text-dark"
                           hx-get="/partials/tasks/table.php?<?= $sortData['url'] ?>"
                           hx-target="#task-list-table"
                           hx-include="#task-search-input, #task-filters select, #task-filters input">
                            Category <i class="bi <?= $sortData['icon'] ?> small"></i>
                        </a>
                    </th>
            ...
            <tbody id="tasks-table-body">
                <?php foreach ($tasks as $task): ?>
                    ...
                    $assigneeName = !empty($task['first_name']) && !empty($task['last_name'])
                        ? htmlspecialchars($task['first_name'] . ' ' . $task['last_name'])
                        : '<span class="text-muted">Unassigned</span>';
                    ?>
                    <tr class="<?= $rowClass ?>" id="task-row-<?= $task['id'] ?>">
                        <!-- Checkbox -->
                        <td>
                            <input type="checkbox"
    ...
    <!-- Pagination -->
    <?php if ($totalPages > 1): ?>
        <nav aria-label="Task list pagination" id="task-pagination">
            <ul class="pagination justify-content-center mb-0">
                ...
                <li class="page-item <?= $page <= 1 ? 'disabled' : '' ?>">
                    <a class="page-link"
                       href="#"
                       <?php if ($page > 1): ?>
                       hx-get="/partials/tasks/table.php?page=<?= $page - 1 ?><?= $queryString ?>"
                       hx-target="#task-list-table"
                       hx-include="#task-search-input, #task-filters select, #task-filters input"
                       <?php endif; ?>>
                ...
                if ($startPage > 1): ?>
                    <li class="page-item">
                        <a class="page-link"
                           href="#"
                           hx-get="/partials/tasks/table.php?page=1<?= $queryString ?>"
                           hx-target="#task-list-table"
                           hx-include="#task-search-input, #task-filters select, #task-filters input">
                            1
                        </a>
                ...
                <?php for ($i = $startPage; $i <= $endPage; $i++): ?>
                    <li class="page-item <?= $i === $page ? 'active' : '' ?>">
                        <a class="page-link"
                           href="#"
                           hx-get="/partials/tasks/table.php?page=<?= $i ?><?= $queryString ?>"
                           hx-target="#task-list-table"
                           hx-include="#task-search-input, #task-filters select, #task-filters input">
                            <?= $i ?>
                        </a>
                ...
                <?php if ($endPage < $totalPages): ?>
                    ...
                    <li class="page-item">
                        <a class="page-link"
                           href="#"
                           hx-get="/partials/tasks/table.php?page=<?= $totalPages ?><?= $queryString ?>"
                           hx-target="#task-list-table"
                           hx-include="#task-search-input, #task-filters select, #task-filters input">
                            <?= $totalPages ?>
                        </a>
